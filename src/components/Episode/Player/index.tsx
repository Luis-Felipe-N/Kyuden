...
const InnerPlayer = forwardRef<HTMLVideoElement, IInnerPlayerProps>(
  ({ children, episode, ...props }, ref) => {
    ...
    const watchedEpisodeData =
      user && episode ? getWatchedEpisodeData(user, episode) : null

    const savePreviosTime = useCallback(() => {
      if (playerRef.current !== null) {
        if (user) {
          updateUserData(user.uid, {
            watchingEpisodes: arrangeAndAddObject(
              user.watchingEpisodes || {},
              {
                id: episode.id,
    ...
    return (
      <VideoProvider
        episode={episode}
        videoRef={playerRef}
        containerPlayerRef={containerPlayerRef}
      >
