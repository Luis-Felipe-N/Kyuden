...
export function EpisodeCardWatched({ episode, anime }: INextEpisodeProps) {
  ...
  const assistedTimeInPercentage =
    (Number(getWatchedEpisode?.assistedTime) * 100) / episode.duration
  ...
  return (
    ...
    >
      <div className={style.episode__cover}>
        ...
        {linkStream ? (
          ...
          <Image
            ...
            alt={
              anime
                ? `Thumbnail do episode ${episode.title} do anime ${anime.title}`
                : `Thumbnail do episode ${episode.title}`
            }
            title={
              anime
                ? `Thumbnail do episode ${episode.title} do anime ${anime.title}`
      ...
      <div className={style.episode__infos}>
        ...
        <span>
          {episode.duration > 0 &&
            convertMillisecondsInMinutes(episode.duration)}
        </span>
      </div>
    </Link>
  )
